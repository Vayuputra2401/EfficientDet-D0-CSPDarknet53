...
class EfficientDetBackbone(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        max_size1 = inputs1.shape[-1]
        max_size2 = inputs2.shape[-1]

        _, p3_1, p4_1, p5_1 = self.backbone_net(inputs1)
        _, p3_2, p4_2, p5_2 = self.backbone_net(inputs2)

        features1 = (p3_1, p4_1, p5_1)
        features1 = self.bifpn(features1)

        features2 = (p3_2, p4_2, p5_2)
        features2 = self.bifpn(features2)

        regression1 = self.regressor1(features1)
        classification1 = self.classifier1(features1)

        regression2 = self.regressor2(features2)
        classification2 = self.classifier2(features2)

        anchors1 = self.anchors(inputs1, inputs1.dtype)
        anchors2 = self.anchors(inputs2, inputs2.dtype)

        return features1, regression1, classification1, anchors1, features2, regression2, classification2, anchors2
